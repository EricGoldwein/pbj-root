...
- **Why we’re confident non-F13 use sa/ALL:** (1) **Verified live:** `https://docquery.fec.gov/cgi-bin/forms/C00892471/1930534/sa/ALL` (MAGA Inc., Form 3X) and `.../C00573949/1943222/sa/ALL` (F3L) return Schedule A. (2) FEC docquery uses “sa” for Schedule A; only Form 13 has path f132. (3) If a link fails with **“Invalid Report Id”**, the cause is the **id** (file_number from API), not the path — OpenFEC’s `file_number` is not always docquery’s report id. See `donor/EXAMPLES_FEC_DOCQUERY_URLS.md` for manual check results.
