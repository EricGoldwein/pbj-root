...
def query_donations_by_committee(
    ...
    while True:
        ...
        try:
            ...
            # Safety: when using page fallback, if this batch's first record matches previous batch's first, API may be returning same page ΓÇö stop
...
def _is_valid_filing_image_id(val: Any) -> bool:
    ...
    look like valid filing numbers (all digits, 4ΓÇô12 chars).
    """
    if val is None:
        return False
    s = str(val).strip()
