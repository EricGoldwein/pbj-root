...
## How residents are attributed to days (high level, CMS framing)

CMS describes building the daily CMG distribution with logic aligned to **daily MDS census** (with adaptations for PDPM nursing CMGs). Outline useful for researcher/attorney tiers — always confirm wording in current **Users Guide**:

1. Define the **reporting quarter** (calendar quarter aligned to the staffing/publication workflow).  
2. Pull **MDS** (including PDPM nursing CMGs) from **one year before** the quarter start through quarter end so late-entry residents can be attributed.  
3. **Discharge / death:** use discharge dates (or death in facility); if a later admission MDS appears, reopen residence from that admission date onward.  
4. **Long gaps:** if **150+ days** pass without an MDS, CMS treats the resident as no longer present from the **150th day after the last assessment** forward (reflecting expected quarterly MDS rhythms).  
5. For each **calendar day**, count residents passing those tests; each resident’s **most recent qualifying assessment** through that date supplies the **PDPM nursing CMG** for that day.
