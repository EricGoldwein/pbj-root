...
class StateOwnerIndexSeoTests(unittest.TestCase):
    ...
    def test_state_ranking_uses_in_state_ownership_interest(self):
        """State OI ranking must use in-state counts, not national contamination."""
        from ownership.state_owner_index import filter_ownership_interest_portfolio_rows

        rows = [
            {
                "associate_id": "1111111111",
                "name": "Mark Hancock",
                "facility_count": 3,
                "facility_count_total": 273,
                "facility_count_ownership_interest": 3,
            },
            {
                "associate_id": "2222222222",
                "name": "Ensign Group",
                "facility_count": 18,
                "facility_count_total": 200,
                "facility_count_ownership_interest": 18,
            },
            {
                "associate_id": "3333333333",
                "name": "Control Only LLC",
                "facility_count": 40,
                "facility_count_total": 40,
                "facility_count_ownership_interest": 0,
            },
        ]
        ranked = filter_ownership_interest_portfolio_rows(rows)
        self.assertEqual([r["name"] for r in ranked], ["Ensign Group", "Mark Hancock"])
        self.assertEqual(ranked[0]["facility_count_rank"], 18)
        self.assertEqual(ranked[1]["facility_count_rank"], 3)
        self.assertEqual(
            format_portfolio_facility_count(
                "TX",
