...
export async function loadAllData(basePath: string = '/data', scope?: 'usa' | 'state' | 'region', identifier?: string): Promise<LoadedData> {
  try {
    ...
    // For USA scope, we ONLY need national Q2 (facility/provider can be empty arrays - old behavior)
    ...
    // For USA scope, only require national Q2 (matches old behavior exactly)
    // For state/region scope, require state/region/national data AND facility/provider JSON
    // Facility/provider JSON is optional for USA - we'll use empty arrays if missing (old behavior)
    const hasJsonData = scope === 'usa'
      ? hasEssentialJsonData  // Only check national Q2 for USA (old behavior)
    ...
    const [
      ...
      stateStandardsCsv ? parseCSV<any>(stateStandardsCsv) : Promise.resolve([]),
    ]);
        ...
        if (minStaffing >= 1.0) {
          ...
          if (stateName) {
            ...
            if (stateCode) {
              ...
              // Debug: Log NY specifically
              if (key === 'ny' || stateName.toLowerCase().includes('new york')) {
                console.log(`[State Standards] Mapped "${stateName}" -> "${key}" with min ${minStaffing}`);
              }
            } else {
              // Debug: Log if state name couldn't be converted
              if (stateName.toLowerCase().includes('new york')) {
                console.warn(`[State Standards] Could not convert state name "${stateName}" to code`);
              }
            }
          }
        }
    ...
    console.log(`[State Standards] Loaded ${stateStandardsMap.size} state standards. NY present: ${stateStandardsMap.has('ny')}`);
