...
OG_W, OG_H = 1200, 630
MAP_W, MAP_H = 860, 560
TITLE = "Nursing Home State Staffing Trends, Q1 2026"
BRAND = "PBJ320"
MIN_BYTES = 40_000
BG = (15, 23, 42)
HEAD_H = 78
...
def _assert_real_map(im: Image.Image, raw_bytes: int) -> None:
    # Blank / failed renders are tiny and nearly mono-chrome navy.
    colors = im.getcolors(maxcolors=200_000)
    n_colors = len(colors) if colors else 200_000
    if raw_bytes < 20_000 or n_colors < 80:
        raise SystemExit(
            f"OG raster looks empty (bytes={raw_bytes}, unique_colors≈{n_colors}). "
            "Aborting so we do not ship a blank LinkedIn image."
        )
...
def main() -> None:
    ...
    _assert_real_map(rendered, len(png_bytes))
    ...
    if size < MIN_BYTES:
        raise SystemExit(f"Wrote {OUT} but filesize {size} < {MIN_BYTES}; refusing ship.")
    print(f"Wrote {OUT} ({OG_W}x{OG_H}, {size} bytes)")
