...
def main():
    """Extract SFF tables A–D from a CMS posting PDF into derived CSVs."""
    repo_root = Path(__file__).resolve().parents[2]
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))
    from sff_paths import SFF_TABLES_DIR, find_latest_raw_pdf
    ...
    if len(sys.argv) > 1:
        ...
        if latest_pdf is None:
            print("Error: No SFF posting PDF found under data_sources/cms/sff/raw/")
            print("Usage: python scripts/sff/extract_sff_posting.py [path/to/sff-posting.pdf]")
            sys.exit(1)
        pdf_path = latest_pdf
