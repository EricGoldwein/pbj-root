...
def owner_page_title(display_name: str, profile: dict[str, Any] | None = None) -> str:
    ...
    if profile:
        # Prefer labels already attached via attach_publication_taxonomy
        # (store segment), then helper that respects publication_segment.
        attached = str(profile.get('publication_title_suffix') or '').strip()
        if attached:
            suffix = attached
        else:
...
def owner_page_meta_description(
    ...
    *,
    ...
    if profile:
        attached = str(profile.get('meta_relationship_phrase') or '').strip()
        if attached:
            rel = attached
        else:
